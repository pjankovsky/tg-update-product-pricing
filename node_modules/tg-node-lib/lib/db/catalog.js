    ...
    let _init = function () {
        return require('tg-node-lib/lib/config').getAllConfig().then((config) => {
            ...
            ProductPrices = db.define('product_price', {
                ...
                instanceMethods: {
                    updateCost: (self, cost, available) => {
                        self.available = available;
                        self.cost = parseInt(cost);
                        self.price = Math.round(self.cost * parseFloat(config['price.margin.default']));
                        self.searsPrice = Math.round(self.cost * parseFloat(config['price.margin.sears']));
                        return self.save();
                    }
                }
            });
